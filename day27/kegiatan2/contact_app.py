import sys
from PyQt5.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QHBayout, QLabel, QLineEdit, QPushButton, QMessageBox, QListWidget
from PyQt5.QtSql import QSqlDatabase, QSqlQuery
from kegiatan2.contactapp import Contact
...
class ContactApp(QMainWindow):
   def __init__(self):
       ...
       if not self.db.open():
           QMessageBox.critical(self, 'Error', 'Tidak dapat terhug ke database.')
           sys.exit(1)

       # Membuat tabel kontak jika belum ada
       query = QSqlQuery()
       query.exec_('CREATE TABLE IF NOT EXISTS contacts (name TEXphone TEXT, email TEXT)')
   ...
   def add_contact(self):
       ...
       query = QSqlQuery()
       query.prepare('INSERT INTO contacts (name, phone, email) VS (?, ?, ?)')
       ...
       if query.exec_():
           self.load_contacts()
           self.clear_inputs()
           QMessageBox.information(self, 'Info', 'Kontak berhasilmbahkan.')
       else:
           QMessageBox.warning(self, 'Peringatan', 'Gagal menambakontak.')
   def update_contact(self):
       ...
       if current_item:
        ...
        if query.exec_():
            self.load_contacts()
            self.clear_inputs()
            QMessageBox.information(self, 'Info', 'Kontak berhasilsil diperbarui.')
        else:
   ...
   def delete_contact(self):
       ...
       if current_item:
           selected_name = current_item.text()
           confirm = QMessageBox.question(self, 'Konfirmasi','f anda yakin ingin menghapus kontak {selected_name}?', QMessageBox
.yes | QMessageBox.No)
